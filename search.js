    ...
    for (const book of window.CONCISE_BOOK_DATA) {
        ...
        for (let i = 0; i < titles.length; i++) {
            ...
            if (normalizedTitle.includes(normalizedInput)) {
                matches.push({ title, url: generateUrl(prefix, i + 1), collectionName: formatBookName(prefix) });
            }
        }
    }
    return matches;
...
function generateUrl(collection, index) {
    return `https://joongangart.kr/${collection}/${index.toString().padStart(2, '0')}/pop1.html`;
}
...
function formatBookName(prefix) {
    ...
    if (prefix.startsWith('ShinSangWooArrange')) {
        const vol = prefix.replace('ShinSangWooArrange', '');
        return vol === '1SSA' ? `신상우 편곡집 Vol.1 (SSA)` : `신상우 편곡집 Vol.${vol}집`;
    }
